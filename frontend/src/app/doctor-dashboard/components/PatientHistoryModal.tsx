...
export default function PatientHistoryModal({ patientId, onClose }: PatientHistoryModalProps) {
    ...
    const [activeTab, setActiveTab] = useState<'medical' | 'appointments' | 'prescriptions'>('medical');
                        ...
                        <>
                            ...
                            <div className="mb-6 p-4 bg-muted/50 rounded-lg">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        {history.patient.chronicConditions && history.patient.chronicConditions.length > 0 && (
                                            <div className="mt-1 text-sm">
                                                <span className="text-warning font-medium">Conditions:</span>{' '}
                                                <span className="text-text-secondary">{history.patient.chronicConditions.join(', ')}</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="flex gap-4 mb-6 border-b border-border">
                                <button
                                    ...
                                    className={`pb-3 px-2 font-medium transition-base relative ${activeTab === 'medical' ? 'text-primary' : 'text-text-secondary hover:text-text-primary'
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`pb-3 px-2 font-medium transition-base relative ${activeTab === 'appointments' ? 'text-primary' : 'text-text-secondary hover:text-text-primary'
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`pb-3 px-2 font-medium transition-base relative ${activeTab === 'prescriptions' ? 'text-primary' : 'text-text-secondary hover:text-text-primary'
                                        }`}
                                >
                                ...
                                <div className="space-y-3">
                                    {history.medicalRecords.length === 0 ? (
                                        ...
                                        history.medicalRecords.map((record) => (
                                            <div key={record.id} className="p-4 bg-muted/30 rounded-lg border border-border">
                                                ...
                                                {record.result && (
                                                    <p className="mt-2 text-sm"><span className="font-medium">Result:</span> {record.result}</p>
                                                )}
                                                {record.notes && (
                                ...
                                <div className="space-y-3">
                                    {history.appointments.length === 0 ? (
                                        ...
                                        history.appointments.map((appt) => (
                                            <div key={appt.id} className="p-4 bg-muted/30 rounded-lg border border-border">
                                                <div className="flex items-start justify-between">
                                                    <div>
                                                        <h4 className="font-medium text-text-primary">
                                                            {formatDate(appt.date)} at {appt.time}
                                                        </h4>
                                                        <p className="text-sm text-text-secondary">{appt.symptoms || 'General consultation'}</p>
                                                    </div>
                                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${appt.status === 'completed' ? 'bg-success/10 text-success' :
                                ...
                                <div className="space-y-3">
                                    {history.prescriptions.length === 0 ? (
                                        ...
                                        history.prescriptions.map((prescription) => (
                                            <div key={prescription.id} className="p-4 bg-muted/30 rounded-lg border border-border">
                                                <div className="flex items-start justify-between mb-2">
                                                    <h4 className="font-medium text-text-primary">
                                                        {prescription.diagnosis || 'Prescription'}
                                                    </h4>
                                                    <span className="text-xs text-text-muted">{formatDate(prescription.createdAt)}</span>
                                                </div>
                                                ...
                                                {prescription.notes && (
                                                    <p className="mt-2 text-sm text-text-secondary italic">{prescription.notes}</p>
                                                )}
                                            </div>
                                        ))
                                    )}
