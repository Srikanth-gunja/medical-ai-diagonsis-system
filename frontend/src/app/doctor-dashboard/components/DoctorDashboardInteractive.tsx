...
export default function DoctorDashboardInteractive() {
    ...
    const [activeTab, setActiveTab] = useState<'appointments' | 'patients' | 'requests' | 'completed'>('appointments');
    ...
    const [chatPatient, setChatPatient] = useState<{ id: string; name: string; image: string } | null>(null);

    // Consultation modal state
    const [showConsultationModal, setShowConsultationModal] = useState(false);
    const [pendingFinishAppointment, setPendingFinishAppointment] = useState<{ id: string; patientName: string } | null>(null);
    const [isCompletingAppointment, setIsCompletingAppointment] = useState(false);

    // Reject modal state
    const [showRejectModal, setShowRejectModal] = useState(false);
    const [pendingRejectRequest, setPendingRejectRequest] = useState<{ id: string; patientName: string } | null>(null);
    ...
    const [patientsWithActiveAppointments, setPatientsWithActiveAppointments] = useState<Set<string>>(new Set());
    const [requests, setRequests] = useState<AppointmentRequest[]>([]);
    const [analytics, setAnalytics] = useState<DoctorAnalytics | null>(null);
    const [chartData, setChartData] = useState<ChartData | null>(null);
    const [isLoading, setIsLoading] = useState(true);
    const [nextAppointment, setNextAppointment] = useState<{ patientName: string; time: string } | null>(null);
        ...
        try {
            ...
            try {
                ...
                for (const a of apptData) {
                    const typeFormatted = (a.type === 'video' ? 'Video' : a.type === 'in-person' ? 'In-Person' : 'Video') as 'Video' | 'In-Person' | 'Phone';
                    const statusFormatted = (a.status.charAt(0).toUpperCase() + a.status.slice(1)) as 'Confirmed' | 'Pending' | 'Completed' | 'Cancelled';
            ...
            try {
                ...
                const formattedPatients: Patient[] = patientsData.map(p => ({
                    ...
                    condition: p.chronicConditions && p.chronicConditions.length > 0 ? p.chronicConditions[0] : 'General Care',
                    status: 'Active'
                }));
        ...
        <div className="min-h-screen bg-background">
            <AuthenticatedHeader
                user={{
                    ...
                    role: 'doctor',
                    avatar: doctorProfile?.image || 'https://images.unsplash.com/photo-1612349317150-e413f6a5b16d'
                }}
            ...
            <div className="container mx-auto px-4 sm:px-6 py-6">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-text-secondary">Manage your appointments, patients, and practice analytics</p>
                </div>
