...
const ChatPanel = ({ isOpen, onClose, patientId, patientName, patientImage }: ChatPanelProps) => {
  ...
  return (
    <>
      ...
      {/* Slide-in Chat Panel */}
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-md bg-card/95 backdrop-blur-xl border-l border-border/50 shadow-[-10px_0_30px_-15px_rgba(0,0,0,0.2)] z-50 transform transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        {/* Gradient Header */}
          ...
          <div className="relative flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <div
                  className={`absolute bottom-0 right-0 w-4 h-4 rounded-full border-2 border-card ${canChat ? 'bg-success animate-pulse' : 'bg-text-muted'}`}
                />
              </div>
              <div>
